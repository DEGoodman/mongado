...
async def get_quick_lists(
    ...
    All three are structural signals. Time-since-edit was deliberately dropped
    (#262): every note in the KB was older than the 60-day threshold, so it
    selected everything and distinguished nothing.
...
async def get_note_of_day(notes_service: NotesDep) -> dict[str, Any]:
    """Get a random note to resurface, for the homepage widget.

    Previously this preferred notes untouched for 60+ days and badged them
    "Needs Review". That was removed in #262: every note in the KB was past
    the threshold, so the filter selected the whole corpus and the badge fired
    on everything. Age is not a defect signal in a Zettelkasten anyway - a
    note left alone while accumulating backlinks is doing its job.

    Structural problems are surfaced by Inspire (#259) and the quick lists.

    Returns:
        Dict with a randomly chosen note
    """
